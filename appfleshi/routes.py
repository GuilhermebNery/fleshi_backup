...
from appfleshi import app, database, bcrypt, socketio
...
def profile(user_id):
    if int(user_id) == int(current_user.id):
        ...
        if photo_form.validate_on_submit():
            ...
            socketio.emit('novo_post', {'mensagem': f'{current_user.username} postou uma nova foto!'})
...
@app.route('/testar_socket')
def testar_socket():
    # Isso vai forçar o envio do sinal sem precisar postar foto
    print(">>> TENTANDO ENVIAR SINAL VIA SOCKET <<<")
    socketio.emit('novo_post', {'mensagem': 'TESTE DE SOM FUNCIONANDO!'})
    return "Sinal enviado! Verifique a outra aba."